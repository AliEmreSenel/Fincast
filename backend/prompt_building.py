...
def create_df(STOCK, startDate, endDate, finnhub_client, api):
    df_company_profile = pd.DataFrame(
        finnhub_client.company_profile2(symbol=STOCK), index=[0]
    )

    df_news = pd.DataFrame(
        finnhub_client.company_news(STOCK, _from=startDate, to=endDate)
    )

    pd.options.display.float_format = "{:,.2f}".format

    IS = requests.get(
        f"https://financialmodelingprep.com/api/v3/income-statement/{STOCK}?apikey={api}"
    ).json()

    Ratios = requests.get(
        f"https://financialmodelingprep.com/api/v3/ratios/{STOCK}?apikey={api}"
    ).json()
...
def build_prompt_from_df(STOCK, startDate, endDate, period=7):
    df_basic_financials, df_news, df_company_profile = create_df(
        STOCK, startDate, endDate, FINNHUB_CLIENT, API
    )
    ...
    attr1 = "EPS"
    value1 = (
        df_basic_financials[df_basic_financials["Year"] == int(date)][attr1].values[0]
        if df_basic_financials[df_basic_financials["Year"] == int(date)][
            attr1
        ].values.size
        > 0
        else None
    ...
    attr2 = "PE"
    value2 = (
        df_basic_financials[df_basic_financials["Year"] == int(date)][attr2].values[0]
        if df_basic_financials[df_basic_financials["Year"] == int(date)][
            attr2
        ].values.size
        > 0
    ...
    value3 = (
        df_basic_financials[df_basic_financials["Year"] == int(date)][attr3].values[0]
        if df_basic_financials[df_basic_financials["Year"] == int(date)][
            attr3
        ].values.size
        > 0
